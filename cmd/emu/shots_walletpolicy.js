// The Wallet Policy journey's walk: gather a wallet policy over NFC, and read
// the PROOF the device shows for it.
//
//   const m = await import("./shots_walletpolicy.js");
//   await m.run({ shotURL, md1, expect });
//
// NOT loaded by index.html — it is driven by design/journeys/capture_walletpolicy.py.
//
...
export async function run({ shotURL = "http://127.0.0.1:8732", md1 = [], expect = {} } = {}) {
  ...
  if (first !== "SeedHammer") {
    taken.push(await screenShot(shotURL, "w00a-payload-prompt.png"));
    await tap(BACK);
    await waitFor("SeedHammer");
  }
  taken.push(await screenShot(shotURL, "w00-boot.png"));
  ...
  taken.push(await screenShot(shotURL, "w01-carousel.png"));
  ...
  taken.push(await screenShot(shotURL, "w02-gather-empty.png"));
  ...
  taken.push(await screenShot(shotURL, "w03-gather-full.png"));

  // (5) Done → the consent screen.
  await tap(CONFIRM);
  await waitFor("Policy-ID");
  const consent = await readAllPages(shotURL, "w04-consent-p");
