// VERIFY WALK — derived from walk_s3_nested.js. Taps "Verify now" (row 0) at
// the verify offer instead of Skip (row 1), to drive gui/multisig_verify.go
// -> bundle.Verify, which NO journey has ever reached.
//
...
/** Master A — BIP-39's published all-zero vector; the cards payload's ClassMnemonic. */
export const MASTER_A_WORDS =
...
export async function run({ payload = "cards", n = 3, k = 2, selfSlot = 0,
  ...
  // (10) TAKE the verify offer: "Verify now" is row 0, "Skip" is row 1.
  // s3_nested taps row 1; this walk taps row 0, which is the whole point --
  // bundle.Verify is the device's own funds check (fingerprint, xpub, origin,
  // md1, ms1 entropy) and no journey has ever driven the GUI through it.
  ...
  // ─── THE READBACK ──────────────────────────────────────────────────────────
  //
  // multisigVerifyFlow re-gathers the PUBLIC cards through bundleGatherFlow
  // (gui/multisig_verify.go), then compares derived-vs-readback in
  // bundle.Verify. Feed it the strings the device just engraved.
  await waitFor("Scan a card, or Done");
  const engraved = JSON.parse(window.shToolpath.strings()).strings;
  const publicCards = engraved.filter((c) => c.startsWith("mk1") || c.startsWith("md1"));
  if (publicCards.length === 0) {
    throw new Error(`nothing to read back: engraved strings were ${JSON.stringify(engraved)}`);
  }
  for (const c of publicCards) { window.shNFC.present(c); await sleep(150); }
  await waitFor("Card added");
  await tap(CONFIRM, 1200);   // Done

  // ─── THE SEED, TYPED ───────────────────────────────────────────────────────
  //
  // Deliberately by hand, not from the payload: gui/multisig_verify.go says a
  // readback taken from the session "would compare the engrave against itself".
  // Master A is BIP-39's own published all-zero vector (cmd/emu/sysw_cards_payload.go).
  await waitFor("Choose number of words");
  await tap([240, rowY(0, 2)], 300);          // 12 WORDS
  await tap(CONFIRM, 400);
  const words = MASTER_A_WORDS.trim().split(/\s+/);
  for (let i = 0; i < words.length; i++) {
    const w = words[i], n = i + 1;
    for (let j = 0; j < w.length; j++) {
      await tap(keyPoint(w[j]), 120);
      const want = `${n}:${w.slice(0, j + 1).toUpperCase()}`;
      if (!squash(window.shScreen()).includes(want)) {
        throw new Error(`typing word ${n} (${w}) letter ${j + 1}: screen lacks ${JSON.stringify(want)}; ` +
          `reads ${JSON.stringify(window.shScreen())}`);
      }
    }
    await tap(CONFIRM, 400);
  }
  ...
  // ─── SKIP THE PASSPHRASE ───────────────────────────────────────────────────
  await waitFor("Add a BIP-39 passphrase?");
  await tap([240, rowY(0, 2)], 300);          // Skip is row 0
  await tap(CONFIRM, 900);

  // ─── TYPE THE ms1 ──────────────────────────────────────────────────────────
  //
  // The LAST input. Verify compares the ms1 RECOVERED ENTROPY (not the string),
  // so the secret card has to be re-entered by hand for the same air-gap reason
  // the seed is.
  //
  // MS1_KEY_ROWS was mapped by probing the live emulator on 2026-08-19, not read
  // off a layout constant: this keyboard adds a DIGITS row, and the letter rows
  // stay at the BIP-39 y positions (a 'q' tap at y=198 registers on both). The
  // digits row sits one pitch above at y=152, x0=87.
  await waitFor("Type ms1");
  const ms1 = engraved.find((c) => c.startsWith("ms1"));
  if (!ms1) throw new Error(`no ms1 among the engraved strings: ${JSON.stringify(engraved)}`);
  for (const ch of ms1) {
    await tap(ms1KeyPoint(ch), 110);
  }
  await tap(CONFIRM, 1200);

  // ─── THE GATE: bundle.Verify's verdict ─────────────────────────────────────
  const verdict = [];
  let settled = null;
  for (let i = 0; i < 60; i++) {
    const scr = squash(window.shScreen());
    verdict.push(String(window.shScreen()).replace(/\s+/g, " ").slice(0, 120));
    if (/verified|mismatch|does ?not ?match|failed/i.test(scr)) { settled = window.shScreen(); break; }
    await tap(CONFIRM, 500);
  }
  if (settled === null) {
...
/** The ms1 keyboard: a DIGITS row above the BIP-39 letter rows. Mapped by
 *  probing the live emulator, 2026-08-19 — see the note at the call site. */
export const MS1_KEY_ROWS = [
...
const MS1_KEY_PITCH = 34;   // same pitch as the BIP-39 keyboard (walk_trace_b.KEY_PITCH)

export function ms1KeyPoint(ch) {
