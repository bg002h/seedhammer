...
// The pin follows testdata/hashlock-v0.8.provenance.json, which
// SPEC_hashlock_kinds phase 4 re-vendored to ms-codec 0.10.0 (commit
...
type corpus struct {
	Derivation []struct {
		...
		// The three kinds SPEC_hashlock_kinds phase 2 added. The columns were
		// in the vendored corpus and NO Go field read them, so three of the
		// four digest functions had no KAT at all (P4 migration, D-1).
		HardenedHash256   string `json:"hardened_h_hash256"`
		HardenedRipemd160 string `json:"hardened_h_ripemd160"`
		HardenedHash160   string `json:"hardened_h_hash160"`
...
func loadCorpus(t *testing.T) corpus {
	...
	if sum := sha256.Sum256(raw); hex.EncodeToString(sum[:]) != corpusSHA256 {
		t.Fatalf("%s hashes to %x, not the pinned %s -- the vendored copy and ms-codec 0.10.0's have drifted",
...
func TestDerivationRowsLockstep(t *testing.T) {
	...
	for _, r := range c.Derivation {
		...
		if h, want := DigestSHA256(&x), mustHex(t, r.HardenedH); h != want {
		...
		if h, want := DigestSHA256(&x2), mustHex(t, r.SHA256H); h != want {
...
func TestKindRowPreimageDigest(t *testing.T) {
	...
	if got, want := DigestSHA256(&x), mustHex(t, c.Kind[0].Digest); got != want {
		t.Fatalf("kind[0] digest: got %x want %x", got, want)
	}
	if h := DigestSHA256(&x); h == x {
...
// TestDigestKAT is the per-kind known-answer test SPEC_hashlock_kinds §10
// requires, and §12 item 6 gates.
//
// WHY IT EXISTS. Core's address vectors derive from the descriptor TEXT and are
// structurally blind to a wrong digest FUNCTION; checking this package against
// the host that shares its design is self-consistency, not a KAT. The rows come
// from the vendored ms-codec corpus, computed in python3 hashlib OUTSIDE both
// implementations -- a row generated by the code it tests is not a KAT.
//
// IT COVERS THE DISPATCH TOO. Four correct functions behind one mis-wired arm of
// DigestOf is the same lost-funds outcome with a different cause, so both are
// asserted here. Before this test three of DigestOf's four arms never ran.
func TestDigestKAT(t *testing.T) {
	c := loadCorpus(t)
	checked := 0
	for _, r := range c.Derivation {
		if r.HardenedX == "" || r.HardenedHash256 == "" {
			continue
		}
		x := mustHex(t, r.HardenedX)
		for _, tc := range []struct {
			kind md.HashKind
			want string
			fn   func() []byte
		}{
			{md.KindSha256, r.HardenedH, func() []byte { d := DigestSHA256(&x); return d[:] }},
			{md.KindHash256, r.HardenedHash256, func() []byte { d := DigestHash256(&x); return d[:] }},
			{md.KindRipemd160, r.HardenedRipemd160, func() []byte { d := DigestRIPEMD160(&x); return d[:] }},
			{md.KindHash160, r.HardenedHash160, func() []byte { d := DigestHash160(&x); return d[:] }},
		} {
			if got := hex.EncodeToString(tc.fn()); got != tc.want {
				t.Errorf("%s: %s: function gave %s, corpus says %s",
					r.Phrase, tc.kind.Token(), got, tc.want)
			}
			// THE DISPATCH, over the same rows.
			if got := hex.EncodeToString(DigestOf(tc.kind, &x)); got != tc.want {
				t.Errorf("%s: %s: DigestOf gave %s, corpus says %s",
					r.Phrase, tc.kind.Token(), got, tc.want)
			}
			// ...and the width, which is what a 20-byte kind gets wrong
			// silently if a caller reaches past Digest().
			if got, want := len(tc.fn()), tc.kind.DigestLen(); got != want {
				t.Errorf("%s: %s: width %d, want %d", r.Phrase, tc.kind.Token(), got, want)
			}
		}
		checked++
	}
	...
	if checked < 8 {
		t.Fatalf("only %d rows carried all four kinds", checked)
	}
}

// TestHashLockIdentityIgnoresPadding pins what Go cannot express in the type
// system: the alloc-gate padding must not be observable.
//
// Rust hit this in phase 1 by DERIVING equality over the fixed array. Go's ==
// on a struct holding a [32]byte compares all 32 bytes and cannot be
// overridden, so md.HashLock is deliberately non-comparable and callers must
// use Equal and MapKey. This test is what makes that contract load-bearing.
func TestHashLockIdentityIgnoresPadding(t *testing.T) {
	d := make([]byte, 20)
	for i := range d {
		d[i] = byte(0xA0 | (i & 0x0F))
	}
	a, ok := md.NewHashLock(md.KindRipemd160, d)
	if !ok {
		t.Fatal("20 bytes is ripemd160's width")
	}
	b, ok := md.NewHashLock(md.KindRipemd160, d)
	if !ok {
		t.Fatal("20 bytes is ripemd160's width")
	}
	if !a.Equal(b) || a.MapKey() != b.MapKey() {
		t.Error("two locks over the same visible digest must be one lock")
	}
	// The same bytes under a DIFFERENT kind are a different lock: §5's point
	// that the kind is part of identity, not a label on it.
	other, ok := md.NewHashLock(md.KindHash160, d)
	if !ok {
		t.Fatal("20 bytes is hash160's width")
	}
	if a.Equal(other) || a.MapKey() == other.MapKey() {
		t.Error("the same 32 bytes mean different things under different kinds")
	}
	// A wrong-width digest is refused at the boundary, never truncated.
	if _, ok := md.NewHashLock(md.KindRipemd160, make([]byte, 32)); ok {
		t.Error("ripemd160 must refuse a 32-byte digest")
	}
