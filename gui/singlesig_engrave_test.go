...
func TestSingleSigEngravePlatesValidate(t *testing.T) {
	...
	plat := newPlatform()
	for _, p := range bundlePlatePlan(engraverParams, cards) {
		labels, plates, err := validateMdmkStrings(plat, p.strs, "", "")
		if err != nil || len(plates) == 0 || len(labels) == 0 {
			t.Fatalf("plate %q (card %d) does not fit any variant: err=%v plates=%d", p.strs, p.cardIdx, err, len(plates))
		}
	}
}
...
func TestSingleSigEngraveLongestMs1Fits(t *testing.T) {
	...
	for _, p := range bundlePlatePlan(engraverParams, cards) {
		if _, pl, err := validateMdmkStrings(plat, p.strs, "", ""); err != nil || len(pl) == 0 {
			t.Fatalf("plate %q does not fit (would abort the bundle): err=%v", p.strs, err)
		}
	}
}
