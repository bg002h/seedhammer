...
// gap) and sortedmulti/multi nested under a combinator. The fork does NOT port
...
// sortedmulti/multi under it is refused; a tap-script tree refuses sortedmulti_a
// on any leaf.
func guardNode(n node, inCombinator bool) error {
	...
	case tagMultiA:
		...
		return nil
	case tagSortedMulti, tagMulti:
		if inCombinator {
			// sortedmulti / multi nested under a combinator → refuse (no renderer).
			return errTemplateUnsupportedShape
		}
		return nil
	case tagWsh:
	...
	case tagTr:
		...
		// sortedmulti/multi (non-_a) leaf there is refused, and sortedmulti_a is
		// refused by its own arm. multi_a is admitted.
	...
	default:
		...
		// inCombinator=true so any sortedmulti/multi below is refused.
		if b, ok := n.body.(childrenBody); ok {
			for _, c := range b.children {
				if err := guardNode(c, true); err != nil {
					return err
